...
RETIRE_AGE = 55                  # อายุรับบำนาญชราภาพ
...
# ---------- edit profile (พับเก็บ) ----------
with st.expander("✏️ แก้ไขข้อมูลของฉัน"):
    with st.form("profile_form"):
        c1, c2, c3 = st.columns(3)
        name   = c1.text_input("ชื่อ-นามสกุล", prof["full_name"])
        age    = c2.number_input("อายุ", 15, 80, int(prof["age"]))
        gender = c3.selectbox("เพศ", GENDER_OPTIONS,
                              index=GENDER_OPTIONS.index(prof["gender"])
                              if prof["gender"] in GENDER_OPTIONS else 2)
        c4, c5, c6 = st.columns(3)
        prov   = c4.selectbox("จังหวัด", PROVINCES,
                              index=PROVINCES.index(prof["province"])
                              if prof["province"] in PROVINCES else 0)
        mattra = c5.selectbox("มาตรา", list(MATTRA_OPTIONS.keys()),
                              index=list(MATTRA_OPTIONS.keys()).index(prof["mattra"]),
                              format_func=lambda k: MATTRA_OPTIONS[k])
        salary = c6.number_input("ฐานเงินสมทบ (บาท/เดือน)", 0, 100000,
                                 int(prof["salary"]), step=500)
        mo = st.slider("จำนวนเดือนที่ส่งเงินสมทบมาแล้ว", 0, 480, months)
        if st.form_submit_button("💾 บันทึก", type="primary"):
            ok, msg = update_profile({
                "full_name": name, "age": int(age), "gender": gender,
                "province": prov, "mattra": mattra,
                "salary": int(salary), "months_contributed": int(mo)})
